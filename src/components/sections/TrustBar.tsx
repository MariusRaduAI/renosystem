...
export default function TrustBar() {
  ...
  return (
    <section className="relative overflow-hidden border-y border-concrete-100/10 bg-concrete-900">
        ...
        <div className="flex shrink-0 animate-marquee-slow items-center gap-12 pr-12 group-hover:[animation-play-state:paused]">
          {items.map((item, i) => (
            <div key={`${item.label}-${i}`} className="flex shrink-0 items-center gap-3 whitespace-nowrap">
              <span className="font-display text-sm font-bold uppercase tracking-wide text-concrete-100">
                {item.label}
              </span>
              <span className="text-xs text-concrete-300">{item.detail}</span>
