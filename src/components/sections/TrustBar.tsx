...
export default function TrustBar() {
  ...
  return (
    <section className="relative overflow-hidden border-y border-ink-100/10 bg-ink-900">
      <div className="h-1.5 w-full stripe-gold opacity-90" aria-hidden="true" />
        ...
        <div className="flex shrink-0 animate-marquee-slow items-center gap-12 pr-12 group-hover:[animation-play-state:paused]">
          {items.map((item, i) => (
            <div key={`${item.label}-${i}`} className="flex shrink-0 items-center gap-3 whitespace-nowrap">
              ...
              <span className="ml-9 h-1 w-1 rounded-full bg-clay-500" aria-hidden="true" />
            </div>
          ))}
        </div>
        <div
