...
export default function Logo({ variant = "dark", showTagline = false, className }: LogoProps) {
  const textColor = variant === "dark" ? "text-ink-100" : "text-ink-950";
  const subColor = variant === "dark" ? "text-ink-300" : "text-ink-600";

  return (
    <div className={clsx("flex items-center gap-3", className)}>
      <svg viewBox="0 0 64 64" className="h-8 w-8 shrink-0" fill="none" aria-hidden="true">
        <path d="M6 28L18 12L30 28H24V28H12V28H6Z" fill="var(--color-clay-500)" />
        <path
      ...
      <div className="leading-none">
        <div className={clsx("whitespace-nowrap font-display font-extrabold tracking-tight text-base sm:text-lg", textColor)}>
          RENOSYSTEM
          <span className="ml-1.5 hidden text-clay-500 text-[0.65em] font-bold tracking-[0.2em] align-middle sm:inline">
            ALL-IN-ONE
          </span>
        </div>
        {showTagline && (
